...
from utils import time_difference_simplifier
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
from simulation import create_simulation
from interactive_brokers import interactive_broker_connection
...
#Manipulate orders, create trades and its statistics


def create_trades(orders):
	# Separate timezone from DateTime and add to another column
	for order in orders:
		date_time = datetime.datetime.strptime(order["DateTime"].split(" ")[0],
		                                       "%Y-%m-%d;%H:%M:%S")
		...
		order["TradePrice"] = float(
		 order["TradePrice"])  # Ensure TradePrice is float
		order["TradeMoney"] = float(
		 order["TradeMoney"])  # Ensure TradeMoney is float
		order["IBCommission"] = float(
		 order["IBCommission"])  # Ensure IBCommission is float
	...
	for order in orders:
		...
		if quantity_sum[symbol] == 0:
			long_short = "LONG" if orders_within_trade[symbol][0][
			 "Open/CloseIndicator"] == "O" and orders_within_trade[symbol][0][
			  "Buy/Sell"] == "BUY" else "SHORT"
			trade_result = sum(order["NetCash"]
			                   for order in orders_within_trade[symbol])
			trade_amount = sum(
			 abs(order["TradeMoney"]) for order in orders_within_trade[symbol]
			 if order["Open/CloseIndicator"] == "O")
			trade_shares_quantity = sum(
			 abs(order["Quantity"]) for order in orders_within_trade[symbol]
			 if order["Open/CloseIndicator"] == "O")
			trade_result_percentage = trade_result / trade_amount
			win_loss = "WIN" if trade_result > 0 else "LOSS"
			trade_fee = sum(order["IBCommission"]
			                for order in orders_within_trade[symbol])
			trade_asset = orders_within_trade[symbol][0]["AssetClass"]
			trade_enter_date = min(order["DateTime"]
			                       for order in orders_within_trade[symbol])
			trade_exit_date = max(order["DateTime"]
			                      for order in orders_within_trade[symbol])
			...
			trades.append({
			 ...
			 win_loss,
			 "Number of Orders":
			 len(orders_within_trade[symbol]),
			 "Quantity":
			 quantity_sum[symbol],
			 "Trade Shares/Contracts Quantity":
			 trade_shares_quantity,
			 "Trade Result":
			 trade_result,
			 "Trade Amount":
			 trade_amount,
			 "Trade Result Percentage":
			 trade_result_percentage,
			 "Trade Fee":
			 trade_fee,
			 "Enter Date":
			 trade_enter_date,
			 ...
			 trade_exit_date,
			 "Holding time":
			 time_difference_simplifier(trade_enter_date, trade_exit_date),
			 "Orders":
			 orders_within_trade[symbol],
			})
...
def create_trades_journal(trades):
	...
	for trade in trades:
		trade_day = trade['Exit Date'].date().isoformat(
		...
		if trade_day in trades_journal:
			...
			trades_journal[trade_day] = {
			 ...
			 0,
			 "Total Gains":
			 trade["Trade Result"] if trade["Trade Result"] > 0 else 0.0,
			 "Total Losses":
			 (trade["Trade Result"] if trade["Trade Result"] < 0 else 0.0),
			 "Total Fee":
			 trade["Trade Fee"],
			 ...
			 0,
			 "Win Rate":
			 1.0 if trade["Win/Loss"] == "WIN" else 0.0,
			 "Number of Wins":
			 1 if trade["Win/Loss"] == "WIN" else 0,
			 "Number of Loss":
			 1 if trade["Win/Loss"] == "LOSS" else 0,
			 "Trade day":
			 trade_day,
			 "Trades": [trade_data]
			}
		...
		trades_journal[trade_day]["Total Result Percentaage"] = trades_journal[
		 trade_day]["Total Result"] / trades_journal[trade_day]["Total Amount"]
		trades_journal[trade_day]["Average Trade Amount"] = trades_journal[
		 trade_day]["Total Amount"] / trades_journal[trade_day]["Number of Trades"]
		trades_journal[trade_day]["Win Rate"] = trades_journal[trade_day][
		 "Number of Wins"] / trades_journal[trade_day]["Number of Trades"]
...
def create_trades_statistics(trades_journal):
	all_result = 0
	all_gains = 0
	all_losses = 0
	all_profit_factor = 0
	...
	for day in trades_journal:
		...
		all_trades += trades_journal[day]["Number of Trades"]
	...
	all_win_rate = all_win / all_trades
	all_days = len(trades_journal)
	average_amount = all_amount / all_trades
	first_trade_date = list(trades_journal.keys())[0] #dates are collect when trade cokmpletes
	last_trade_date = list(trades_journal.keys())[-1]

	trades_statistics = {
	 "Report date": f'From {first_trade_date} to {last_trade_date}',
	 "Net Result": all_result,
	 "Net Gains $": all_gains,
	 "Net Losses $": -all_losses,
	 "Return %": all_result / all_amount,
	 "Average Amount": average_amount,
	 "Average Gain %": (all_gains / all_win) / average_amount,
	 "Average Loss %": -(all_losses / all_loss) / average_amount,
	 ...
	 "# Loss": all_loss,
	 "Fees": all_fee,
	 "Average Trades per day": all_trades / all_days,
	 "# Days with trades": all_days,
	 "% Days with trades": all_days / pd.date_range(start=first_trade_date, end=last_trade_date, freq='B').size
...
trades_statistics = create_trades_statistics(trades_journal)
print(json.dumps(trades_statistics, indent=2))
...
def create_chart_trade_result_by_day(trades_journal):
	...
	trades_df["Color"] = trades_df["Total Result"].apply(lambda x: "#00FF00"
	                                                     if x >= 0 else "#FF0000")

	# Create the chart using pandas plot
	trades_df.plot(x="index",
	               y="Total Result",
	               kind="bar",
	               title="Total Result by Trade Day",
...
def create_chart_cumulative_trade_result_by_day(trades_journal):
	...
	trades_df["Color"] = trades_df["Cumulative Result"].apply(
...
def create_chart_calendar_heatmap_trade_result_by_day(trades_journal):
	...
	heatmap_trade_result_data = df.pivot(index='Week',
	                                     columns='Day',
	                                     values='Total Result').fillna(0)
	trade_days_data = df.pivot(index='Week', columns='Day',
	                           values='Trade day').fillna('')
	number_of_trades_data = df.pivot(index='Week',
	                                 columns='Day',
	                                 values='Number of Trades').fillna(0)
	...
	available_days = [
	 day for day in days_of_week if day in heatmap_trade_result_data.columns
	]
	...
	sns.heatmap(heatmap_trade_result_data,
	            cmap=cmap,
	            center=0,
	            annot=True,
	            fmt=".2f",
	...
	# Adjust annotations
	for text, trade_day, num_trades in zip(ax.texts,
	                                       trade_days_data.values.ravel(),
	                                       number_of_trades_data.values.ravel()):
...
create_simulation(simulations=10000,
                  investment=2600, # Only real number 
